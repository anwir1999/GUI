...
uint32_t timeDebounce = 0, timeDebounceRotacy = 0, timeCounter = 0, timeDisplayVL= 0,timeRadioFunc = 0;
...
// hien thi menu cu the cua lcd_pointer
void indicateMenu(scr_pointer_t lcd_pointer)
{
  ...
  if(broad_type != BS_NOPE)
  {
    ...
    {
      case BS_INTER:
        broadcastRadio(internet_channel[channelSub -1], IR, broad_type);
        break;
      case BS_DAB:
        broadcastRadio(dab_channel[channelSub -1],skytune, broad_type);
        break;
      case BS_FM:
        broadcastRadio(fm_channel[channelSub -1], radio_dab, broad_type);
        break;
    }
  }
...
void broadcastRadio(radio_channel_t radio_channel, const unsigned short *image, broad_type_t broad_type)
{
    String channel_name;
    channel_name = radio_channel.channel_name;
    broadcast_channel_now = radio_channel;
    ...
    // load lai thoi gian dang phat song
    timeBroadCastIndicate();
}
...
void timeCounterGui()
{
  if(millis() - timeCounter > 1000)
  {
    ...
    Serial.println(SI4684_GetPartInfo());
    SPI_EndTransaction();
  }
}
// hien thi thoi gian broad cast
void timeBroadCastIndicate()
{
   int x = 215;
...
void funcActiveGui()
{
  if(millis() - timeRadioFunc > 1000)
  {
    timeRadioFunc = millis();
    if(status_power == RADIO_ON)
    {
      // wifi connect 
      Wifi_GetInfomation();
      ...
      if(broad_type != BS_NOPE)
      {    
         // hien thi dem thoi gian phat song cua man hinh broadcast
         timeBroadCastIndicate();
      }
      if(flag_fm_scan)
      {
...
void displaySignal(int status_WiFi)
{
  ...
  if(broad_type == BS_NOPE)
  {
    ...
    int x = 240, y = 0;
    if (RTC_Time.Hour < 10) x += frame_30.drawChar('0', x, y, 2); // Add timeline.time_hours leading zero for 24 hr clock
    x += frame_30.drawNumber(RTC_Time.Hour, x, y, 2);             // Draw timeline.time_hours
    x += frame_30.drawChar(':', x, 5, 2);
    if (RTC_Time.Min < 10) x += frame_30.drawChar('0', x, y, 2); // Add timeline.time_minutess leading zero
    x += frame_30.drawNumber(RTC_Time.Min, x, y, 2);             // Draw timeline.time_minutess
  }
  else
  {
